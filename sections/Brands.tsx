...
const Brands = React.memo(function Brands() {
  return (
    <section className="w-full py-8 bg-white overflow-hidden content-visibility-auto">
          ...
          {[...Array(2)].map((_, i) => (
            <div key={i} className="flex gap-20 items-center">
              {brandLogos.map((brand) => (
                <div key={brand.name} className="w-40 h-24 flex items-center justify-center flex-shrink-0">
                  <Image
                    src={brand.logo}
                    alt={brand.name}
                    width={160}
