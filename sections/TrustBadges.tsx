...
const badges = [
  { icon: Shield, title: '25-Year Warranty', description: 'Comprehensive warranty on all solar panels and inverters' },
  { icon: CheckCircle, title: 'Certified Installers', description: 'MNRE approved and trained installation team' },
  { icon: Award, title: 'Premium Brands', description: 'Top-tier panels from Tata, Adani, Waaree & more' },
  { icon: Clock, title: '24/7 Support', description: 'Round-the-clock maintenance and customer support' },
]
...
const TrustBadges = () => {
  ...
  return (
    <section className="py-16 bg-gradient-to-b from-white to-primary-50 content-visibility-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto px-6">
        {badges.map((badge, index) => (
          <BadgeCard key={badge.title} badge={badge} index={index} reduceMotion={shouldReduceMotion} />
        ))}
      </div>
    </section>
  )
